...
module.exports = {
    ...
    if (interaction.customId === "ticket3modal") {

        const q1 = interaction.fields.getTextInputValue('q1');
        const q2 = interaction.fields.getTextInputValue('q2');
        const q3 = interaction.fields.getTextInputValue('q3');
        const staffRole = client.config.ticket3ticketAccesRoleID;
        const category = client.config.ticket3ticketCategoryOpen;
        ...
        if (data) {
            ...
            interaction.reply({embeds: [embedError], ephemeral: true})
                ...
                const ticketChannel = await interaction.guild.channels.create({
                    name: `${client.config.ticket3ticketName}${interaction.user.tag}`,
                    ...
                    const embedChannel = new EmbedBuilder()
                        ...
                        .addFields(
                            {name: `${client.config.ticket3question1Label}`, value: `${q1}`, inline: false},
                            {name: `${client.config.ticket3question2Label}`, value: `${q2}`, inline: false},
                            {name: `${client.config.ticket3question3Label}`, value: `${q3}`, inline: false},            
                        )
                        .setTimestamp()
                        .setFooter({text: 'Dev By: marcoooooooooo_oo'})
                        .setColor('Blue')
