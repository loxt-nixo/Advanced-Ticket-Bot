...
module.exports = {
    ...
    if (interaction.customId === "ticket1modal") {

        const q1 = interaction.fields.getTextInputValue('q1');
        const q2 = interaction.fields.getTextInputValue('q2');
        const q3 = interaction.fields.getTextInputValue('q3');
        const staffRole = client.config.ticket1ticketAccesRoleID;
        const category = client.config.ticket1ticketCategoryOpen;

        if (!staffRole || !category) return console.log('[TICKET 1] Category or role may be incorrect, please check')
        ...
        if (data) {
            ...
            interaction.reply({embeds: [embedError], ephemeral: true})
                ...
                const ticketChannel = await interaction.guild.channels.create({
                    name: `${client.config.ticket1ticketName}${interaction.user.tag}`,
                    ...
                    const embedChannel = new EmbedBuilder()
                        ...
                        .addFields(
                            {name: `${client.config.ticket1question1Label}`, value: `${q1}`, inline: false},
                            {name: `${client.config.ticket1question2Label}`, value: `${q2}`, inline: false},
                            {name: `${client.config.ticket1question3Label}`, value: `${q3}`, inline: false},            
                        )
                        .setTimestamp()
                        .setFooter({text: 'Dev By: marcoooooooooo_oo'})
                        .setColor('Blue')
